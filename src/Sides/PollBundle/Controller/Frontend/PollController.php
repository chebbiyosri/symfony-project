...
namespace Sides\PollBundle\Controller\Frontend;
...
class PollController extends Controller
{
    ...
    public function init()
    {
        ...
        $this->opinionEntity = $this->container->getParameter('sides_poll.opinion_entity');
        ...
        $this->voteForm = $this->container->getParameter('sides_poll.vote_form');
    }
...
//        );
        ...
        return $this->render('SidesPollBundle:Frontend\Poll:list.html.twig', array(
            'polls' => $polls
        ));
    }
    ...
    public function voteAction(Request $request, $pollId)
    {
        ...
        if ($this->hasVoted($request, $pollId)) {
            return $this->forward('SidesPollBundle:Frontend\Poll:results', array('pollId' => $pollId, 'hasVoted' => true));
        }
        ...
        if ('POST' == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                if (!$request->isXmlHttpRequest()) {
                    $response = new RedirectResponse($this->generateUrl('SidesPollBundle_frontend_poll_list'));

                // Show the results
                } else {
                    $response = $this->forward('SidesPollBundle:Frontend\Poll:results', array('pollId' => $pollId, 'hasVoted' => true));
                }
        ...
        return $this->render('SidesPollBundle:Frontend\Poll:vote.html.twig', array(
            'poll' => $poll,
    ...
    public function resultsAction($pollId, $hasVoted = false)
    {
        ...
        return $this->render('SidesPollBundle:Frontend\Poll:results.html.twig', array(
            'poll' => $poll,
            'hasVoted' => $hasVoted
        ));
    }
    ...
    {
        return $response->headers->setCookie(new Cookie('sides_poll_' . $pollId, true, time()+3600*24*365));
    }
    ...
    protected function hasVoted(Request $request, $pollId)
    {
        ...
        if ($cookies->has('sides_poll_' . $pollId)) {
            return true;
        }
